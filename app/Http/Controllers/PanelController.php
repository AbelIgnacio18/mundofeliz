...
public function index(Request $request)
{
    ...
    $usuarios = User::all();
    $sedesIds = auth()->user()->getSedesIds(); // IDs de sedes del usuario
    $anolect = Anolectivo::where('estado', 1)->first();
$estudiante = Matricula::where('idanolectivo', $anolect->id)
    ->when(!auth()->user()->esSuperAdmin(), function($query) use ($sedesIds) {
        $query->whereIn('idsede', $sedesIds);
    })
    ->get();
    ...
    $mesesporcentaje = DB::table('meses as me')
        ...
        ->when(!auth()->user()->esSuperAdmin(), function($query) use ($sedesIds) {
            $query->whereIn('m.idsede', $sedesIds);
        })
    ...
    $pagosarticulos = DB::table('pagos as p')
        ...
        ->groupBy('art.nombre', 'c.nombre', 'p.idanolectivo')
        ->get();

    // Asistencias de hoy
    $puntualHoy = DB::table('asistenciaests as a')
        ->join('matriculas as m', 'a.idmatricula', '=', 'm.id')
        ->whereDate('a.fechaentrada', Carbon::today())
        ->where('a.estado', 1)
        ->when(!auth()->user()->esSuperAdmin(), fn($q) => $q->whereIn('m.idsede', $sedesIds))
        ->count();

    $tardeHoy = DB::table('asistenciaests as a')
        ->join('matriculas as m', 'a.idmatricula', '=', 'm.id')
        ->whereDate('a.fechaentrada', Carbon::today())
        ->where('a.estado', 0)
        ->when(!auth()->user()->esSuperAdmin(), fn($q) => $q->whereIn('m.idsede', $sedesIds))
    ...
    $faltaHoy = DB::table('asistenciaests as a')
        ->join('matriculas as m', 'a.idmatricula', '=', 'm.id')
        ->whereDate('a.fechaentrada', Carbon::today())
        ->where('a.estado', 4)
        ->when(!auth()->user()->esSuperAdmin(), fn($q) => $q->whereIn('m.idsede', $sedesIds))
    ...
    $pagosadministrativos = DB::table('pagos as p')
        ...
        ->where('p.idanolectivo', $anolect->id)
        ->groupBy('con.concepto', 'p.idanolectivo')
        ->get();
    ...
    $pagospensiones = DB::table('pagos as p')
        ...
        ->where('con.id', 1)
        ->where('p.idanolectivo', $anolect->id)
        ->groupBy('con.concepto', 'con.id', 'p.idanolectivo')
        ->get();

    // Pagos ventas totales y mes
    $pagosventas = DB::table('pagos as p')
        ->select(DB::raw('sum(p.montototal) as montototal'))
        ->where('p.idanolectivo', $anolect->id)
        ->get();

    $pagosventasmes = DB::table('pagos as p')
        ->select(DB::raw('sum(p.montototal) as montototal'))
        ->where('p.idanolectivo', $anolect->id)
        ->whereMonth('created_at', date('m'))
        ->get();

    // Ingresos totales y mes
    $pagosingresos = DB::table('ingresos as i')
        ->select(DB::raw('sum(i.montototal) as montototal'))
        ->get();

    $pagosingresosmes = DB::table('ingresos as i')
        ->select(DB::raw('sum(i.montototal) as montototal'))
        ->whereMonth('created_at', date('m'))
        ->get();

    // Fechas
    $fechaInicio = $request->filled('fechainicio') ? Carbon::parse($request->fechainicio) : Carbon::parse($anolect->inicio);
    $fechaFin = $request->filled('fechafin') ? Carbon::parse($request->fechafin) : Carbon::today();
    ...
    $reportetarde = DB::table('asistenciaests as a')
        ...
        ->select(
            ...
            DB::raw('(SUM(CASE WHEN a.estado = 0 THEN 1 ELSE 0 END) * 100.0 / COUNT(*)) as porcentaje_tardanza')
        )
        ->groupBy('e.id', 'e.nombre', 'e.apellidos', 'e.celular', 'au.nivel', 'au.grado', 'au.seccion')
    ...
    $asistenciaPorcentaje = DB::table('asistenciaests as a')
        ...
        ->select(
            ...
            DB::raw('(SUM(CASE WHEN a.estado = 1 THEN 1 ELSE 0 END) * 100.0 / COUNT(*)) as porcentaje_asistencia')
        )
        ->groupBy('e.id', 'e.nombre', 'e.apellidos', 'e.celular', 'au.nivel', 'au.grado', 'au.seccion')
    ...
    // Datos para torta (hoy)
    $nivel = $request->nivel ?? 'Inicial';

    $datos = DB::table('asistenciaests as a')
        ->join('matriculas as m', 'a.idmatricula', '=', 'm.id')
        ->where('a.fechaentrada', date('Y-m-d'))
        ->where('m.idaula', $nivel)
        ->when(!auth()->user()->esSuperAdmin(), fn($q) => $q->whereIn('m.idsede', $sedesIds))
        ->select(

...
use App\Models\Pagoegreso;
use App\Http\Requests\StorePagoegresoRequest;
use App\Http\Requests\UpdatePagoegresoRequest;
use Illuminate\Support\Facades\DB;

class PagoegresoController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        if ($request) {
            ...
            $pago = DB::table('pagos as p')
                ...
                ->select('p.id', 'p.idestudiante', 'p.descripcion', 'p.fecha', 'p.montototal', 'e.nombre', 'e.apellidoP', 'e.apellidoM', 'e.dni')
                ->orderBy('id', 'desc')->get();
    ...
    public function show($id)
    {
        $estudiante = DB::table('pagos as p')
            ->join('estudiantes as e', 'p.idestudiante', '=', 'e.id')
            ->select('p.id', 'e.nombre', 'e.apellidoP', 'e.apellidoM', 'e.dni', 'p.created_at as fecha', 'p.montototal')->where('p.id', $id)->get();
        // dd($estudiante);
        $pension = DB::table('pagos as p')
            ->join('estudiantes as e', 'p.idestudiante', '=', 'e.id')
            ->join('pensions as pen', 'p.id', '=', 'pen.idpago')
            ->join('conceptos as c', 'pen.idconcepto', '=', 'c.id')
            ->select('p.id', 'e.nombre', 'e.apellidoP', 'e.apellidoM', 'e.dni', 'c.concepto', 'p.created_at as fecha', 'p.montototal', 'pen.cantidad', 'pen.monto')->where('p.id', $id)->get();
        // dd($pension);
    ...
    public function reportepdf($id)
    {
        ...
        $pension = DB::table('pagos as p')
            ...
            ->join('pensions as pen', 'p.id', '=', 'pen.idpago')
            ->join('conceptos as c', 'pen.idconcepto', '=', 'c.id')
            ->select('p.id', 'c.codigo', 'c.concepto', 'c.monto', 'p.created_at as fecha', 'p.montototal', 'pen.cantidad', 'pen.monto')->where('p.id', $id)->get();
        // dd($pension);

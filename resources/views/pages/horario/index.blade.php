...
@section('content')
    ...
    <div class="card-body p-0">
        <div class="table-responsive mt-4">
                ...
                <tbody>
                    ...
                    @foreach ($horarios as $docenteId => $dias)
                        <tr>
                            ...
                            <td>
                                {{ optional($dias->where('dia_semana', 'lunes')->first())->hora_ingreso ?? '-' }}
                            </td>

                            <td>
                                {{ optional($dias->where('dia_semana', 'martes')->first())->hora_ingreso ?? '-' }}
                            </td>

                            <td>
                                {{ optional($dias->where('dia_semana', 'miercoles')->first())->hora_ingreso ?? '-' }}
                            </td>

                            <td>
                                {{ optional($dias->where('dia_semana', 'jueves')->first())->hora_ingreso ?? '-' }}
                            </td>

                            <td>
                                {{ optional($dias->where('dia_semana', 'viernes')->first())->hora_ingreso ?? '-' }}
                            </td>

                            <td>
                                <div class="flex align-items-center list-user-action">
                                    <a class="btn btn-sm btn-icon text-warning" data-bs-toggle="modal"
                                        data-bs-original-title="Editar" data-bs-target="#model-edit-{{ $docenteId }}">
                                    ...
                                    <a class="btn btn-sm btn-icon text-danger" data-bs-toggle="modal"
                                        ...
                                        data-bs-target="#model-delete-{{ $docenteId }}">
                                        <span class="btn-inner">
                                            <svg width="24" viewBox="0 0 24 24" fill="none"
            ...
            @foreach ($horarios as $docenteId => $dias)
                @include('pages.horario.modal')
                @include('pages.horario.edit')
            @endforeach
        </div>
    </div>
@endsection
